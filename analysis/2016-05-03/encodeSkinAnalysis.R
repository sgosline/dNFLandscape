...
##step 2, remove values below a particular FPKM, let's say 0.1
...
##step 3, normalize
require(limma)
comb.norm=data.frame(voomWithQualityWeights(comb[expr,],normalize.method='quantile')$E)
...
ggplot(subset(full.df,Gene=='BRAF'))+geom_bar(mapping=aes(x=Sample,y=Expression,fill=Source),stat='identity')
ggsave('BRAF_expression_sample.png')


ggplot(subset(full.df,Gene=='BRAF'))+geom_boxplot(mapping=aes(x=Source,y=Expression,fill=Source))
ggsave('BRAF_expression_source.png')

ggplot(subset(full.df,Gene=='HRAS'))+geom_bar(mapping=aes(x=Sample,y=Expression,fill=Source),stat='identity')
ggsave('HRAS_expression_sample.png')


ggplot(subset(full.df,Gene=='HRAS'))+geom_boxplot(mapping=aes(x=Source,y=Expression,fill=Source))
ggsave('HRAS_expression_source.png')


ggplot(subset(full.df,Gene=='MAP2K1'))+geom_bar(mapping=aes(x=Sample,y=Expression,fill=Source),stat='identity')
ggsave('MAP2K1_expression_sample.png')


ggplot(subset(full.df,Gene=='MAP2K1'))+geom_boxplot(mapping=aes(x=Source,y=Expression,fill=Source))
ggsave('MAP2K1_expression_source.png')
...
fit <- eBayes(fit)
full.tab <- topTable(fit, coef='SourceENCODESkin',number=Inf,adjust.method='BY')
write.table(full.tab,file=paste('diffExGenesBetweenDermalAndAllENCODE.txt',sep=''))

source.tab <- topTable(fit, coef='SourceENCODESkin',number=Inf,adjust.method='BY',p.value=10e-4,lfc=1)
write.table(source.tab,quote=F,file=paste('diffExGenesBetweenDermalAndAllENCODE_10e-4_lfc1.txt',sep=''))
...
write.table(rownames(source.tab),file='diffEx10e-3lfc1InDermals.txt',quote=F,row.names=F,col.names=F)

lib.tab <- topTable(fit, coef='LibraryRNA',number=Inf,adjust.method='BY')
write.table(lib.tab,file=paste('diffExGenesBetweenLibrary.txt',sep=''))

comb.tab<-topTable(fit, coef=c('SourceENCODESkin','LibraryRNA'),number=Inf,adjust.method='BY')
write.table(comb.tab,file=paste('diffExGenesBetweenDermalAndAllENCODEWithLibrary.txt',sep=''))
...
write.table(pa.tab,file=paste('diffExGenesBetweenDermalAndPolyAENCODE.txt',sep=''))


rice.list<-c('S100B','UCHL1','ENO2','RET','GFRA2','GFRA3','ARTN','NRTN','GAP43','GDNF',
             'GFRA1','GFRA4','PSPN','NGF','BDNF','NTF4','NTF3','NTSR1','NTSR2','SORT1','NEFH','CALCA','NF1')
RiceList<-c(rep('POS',10),rep('NEG',13))
names(RiceList)<-rice.list
pheatmap(expr.mat[intersect(rice.list,rownames(expr.mat)),],annotation_col=pat.df[,c('Library','Gender','Source')],annotation_row=data.frame(RiceList),file='RiceGenesInData.png',cellwidth=10,cellheight=10)
