...
p1_files=c() #p1_nf1$file
 ...
 p05='syn6097853'
 p1='syn6099307'
 this.script='https://raw.githubusercontent.com/sgosline/dNFLandscape/master/analysis/2016-05-27/doWgsPlots.R'
 
##upload files to syn6126468
synStore(File(p05_nf1$file,parentId='syn6126468'),used=list(list(entity=p05)),executed=list(list(url=this.script)))
synStore(File(p1_nf1$file,parentId='syn6126468'),used=list(list(entity=p1)),executed=list(list(url=this.script)))
...
res.s=plotMutsAcrossSamples(subset(expr.gene.muts1,Status%in%c("StrongSomatic","LikelySomatic")&Gene%in%cancer.genes$Gene.Symbol),minVal=2,prefix='SommutsCosmicP1')

res05.s=plotMutsAcrossSamples(subset(expr.gene.muts05,Status%in%c("StrongSomatic","LikelySomatic")&Gene%in%cancer.genes$Gene.Symbol),minVal=2,prefix='SommutsCosmicP05')
...
pid='syn6126516'
for(f in p05_files)
  synStore(File(f,parentId=pid),used=list(list(entity=p05)),executed=list(list(url=this.script)))

pid='syn6126516'
for(f in p1_files)
  synStore(File(f,parentId=pid),used=list(list(entity=p1)),executed=list(list(url=this.script)))
