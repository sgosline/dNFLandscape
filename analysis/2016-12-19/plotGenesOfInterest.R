...
library(ggplot2)
this.script='https://raw.githubusercontent.com/sgosline/dNFLandscape/master/analysis/2016-12-19/plotGenesOfInterest.R'
p05='syn6097853'
p1='syn6099307'
...
plotGeneAcrossScores<-function(patient.sample.vars,patient.sample.muts,geneName,testName){
  ...
  pval<-'1.0'
  ##calculate p-value
  try(pval<-format(t.test(score~geneMutated,data=df)$p.value,digits=4))
  ##make color by patient and shape by mutation
  p<-ggplot()+geom_boxplot(data=df,aes(x=geneMutated,y=score))+geom_jitter(data=df,aes(x=geneMutated,y=score,shape=geneMutated,color=patient))+ggtitle(paste(geneName,'mutation status by\n',testName,'scores\np=',pval))
  print(p)
