...
#mutType<-'germline'
 for(mt in c("HIGH","HIGH_MODERATE","HIGH_MODERATE_LOW")){
      files=c(files,getCountsFromTable(expr.gene.muts05,mutationType=mutType,sampOrPat=ifelse(length(mutType)==1,'patient','sample'),prefix='p05'))
      files=c(files,getCountsFromTable(expr.gene.muts1,mutationType=mutType,sampOrPat=ifelse(length(mutType)==1,'patient','sample'),prefix='p1'))
